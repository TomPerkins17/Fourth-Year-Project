...
class SingleNoteTimbreCNNSmall(SingleNoteTimbreCNN):
    def __init__(self):
        ...
        self.cnn_layers = nn.Sequential(
            # Deep and narrow architecture - reduced size of convolutional units to 3x3
            # Fewer parameters overall
            # Wider pooling applied to the first axis (e.g. 4x1) to render the network more pitch invariant.
            # Input size: (1, 300, 222)
            # conv1: 1x300x222 to 8x149x110
            nn.Conv2d(in_channels=1, out_channels=8, kernel_size=(3, 3), bias=False),
            nn.BatchNorm2d(8),
            nn.MaxPool2d(kernel_size=2),
            nn.ReLU(),
            # conv2: 8x149x109 to 16x49x54
            nn.Conv2d(in_channels=8, out_channels=16, kernel_size=(3, 3), bias=False),
            nn.BatchNorm2d(16),
            nn.MaxPool2d(kernel_size=(3, 2)),
            nn.ReLU(),
            # conv3: 16x49x53 to 32x15x26
...
class MelodyTimbreCNNSmall(MelodyTimbreCNN):
    def __init__(self):
        super(MelodyTimbreCNN, self).__init__()
